...
const StudentModal = () => {
  ...
  const handleSearchChange = event => {
    ...
    const filteredData = filterData(originalData, value, classFilter);
    setData(filteredData);
  };
  const filterData = (data, searchVal, classFilterVal) => {
    console.log("Filtering with class id:", classFilterVal);
    ...
    return data.filter(user =>
      ...
      (classFilterVal ? user.classe.id === classFilterVal : true) // Assurez-vous que user.classe.id est correct
    );
  };
  ...
  useEffect(() => {
    const filteredData = filterData(originalData, searchValue, classFilter);
    setData(filteredData);
  }, [searchValue, classFilter, originalData]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await MyRequest(`eleves`, 'GET', {}, {'Content-Type': 'application/json'});
        const response2 = await MyRequest(`classes`, 'GET', {}, {'Content-Type': 'application/json'});
