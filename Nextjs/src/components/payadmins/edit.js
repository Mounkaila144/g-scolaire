...
const EditModal = ({data, open, setOpen, setSuccess, setLoading, setError}) => {
  ...
  const [professeur, setProfesseur] = useState(data?.user?.id || '');
  ...
  const onSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await MyRequest('payadmins/' + data.id, 'PUT', formData, {'Content-Type': 'application/json'});
