...
import Add from "../../../components/payadmins/add";
import EditModal from "../../../components/payadmins/edit";
...
const depenseModal = ({}) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: ({ row }) => {
        ...
        return (
          <Button variant='outlined' size='small'  onClick={() => handleViewteach(row.user)}>
            {row.user.nom}
          </Button>
        );
      },
...

    {
      ...
      renderCell: ({ row }) => {
        ...
        const Submitremove = () => {
          ...
          setLoading(true)
          MyRequest('payadmins/'+row.id, 'DELETE', {'data':data}, {'Content-Type': 'application/json'})
            .then(async (response) => {
  ...
  return (
    <Grid container spacing={6}>
      ...
      <ViewModal data={dataUser} open={openview} setOpen={setOpenview} />
      {/*View user modal */}
